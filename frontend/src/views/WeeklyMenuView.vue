<template>
  <main class="weekly-menu-page">
    <header class="weekly-menu-heading">
      <div>
        ...
        <h1>一周菜单</h1>
        <p>手动安排使用已保存菜谱；AI 随机安排会独立推荐本周餐次，保存后自动汇总采购食材。</p>
      </div>
    ...
    <section class="weekly-menu-summary" aria-label="本周菜单摘要">
      ...
      <div class="summary-actions">
        ...
        <el-button
          ...
          :disabled="loading || recipesLoading || saving"
        ...
        <el-button
          type="primary"
          :loading="saving"
          :disabled="loading || recipesLoading || !availableRecipes.length || autoGenerating"
          @click="saveMenu"
        >
    ...
    <section class="weekly-menu-workspace" v-loading="loading || recipesLoading" aria-label="一周菜单工作区">
      <el-empty v-if="!recipesLoading && !availableRecipes.length && !assignedCount" description="还没有可用菜谱">
        <button v-if="embedded" type="button" class="empty-action" @click="emit('open-feature', 'recipes')">
          <BookOpen :size="16" aria-hidden="true" />
          <span>去我的菜谱添加选择，或点击 AI 自动安排</span>
        </button>
          ...
          <div class="menu-grid">
            ...
            >
              ...
              <div v-for="meal in WEEKLY_MEAL_OPTIONS" :key="meal.value" class="meal-slot">
                ...
                >
                  <el-option
                    v-for="recipe in prioritizeRecipesForMeal(availableRecipes, meal.value)"
                    :key="recipe.id"
                    :label="recipe.title"
  ...
  <el-dialog
    ...
    :close-on-click-modal="!autoGenerating"
    :close-on-press-escape="!autoGenerating"
    :show-close="!autoGenerating"
  >
    <p class="auto-generate-dialog-intro">库存安排会使用已保存菜谱；随机安排会独立检索早餐、午餐和晚餐推荐，不读取任何历史记录。</p>
    <div class="auto-generate-options" role="radiogroup" aria-label="AI 菜单安排方式">
      <button
        type="button"
        class="auto-generate-option"
        :class="{ selected: autoGenerateMode === 'PANTRY' }"
        :disabled="!recipes.length"
        role="radio"
        :aria-checked="autoGenerateMode === 'PANTRY'"
        :aria-disabled="!recipes.length"
        @click="autoGenerateMode = 'PANTRY'"
      >
        <span class="auto-generate-option-mark" aria-hidden="true">库存</span>
        <span>
          <strong>根据食材库存安排</strong>
          <small>{{ recipes.length ? '结合“我的食材”中已有的库存，优先安排能直接使用的已保存菜谱。' : '暂无已保存菜谱，暂不能使用库存安排。' }}</small>
        </span>
      </button>
      <button
        type="button"
        class="auto-generate-option"
        :class="{ selected: autoGenerateMode === 'RANDOM' }"
        role="radio"
        :aria-checked="autoGenerateMode === 'RANDOM'"
        @click="autoGenerateMode = 'RANDOM'"
      >
        <span class="auto-generate-option-mark" aria-hidden="true">随机</span>
        <span>
          <strong>随机安排</strong>
          <small>不读取任何用户历史数据；AI 会独立检索早餐、午餐和晚餐推荐后生成本周菜谱，早餐优先包子、面条等主食。</small>
        </span>
      </button>
    </div>
    <template #footer>
      <el-button :disabled="autoGenerating" @click="autoGenerateDialogVisible = false">取消</el-button>
      <el-button type="primary" :loading="autoGenerating" @click="confirmAutoGenerate">开始安排</el-button>
    </template>
  </el-dialog>
...
const recipes = ref([])
const weeklyGeneratedRecipes = ref([])
const recipesLoading = ref(false)
const loading = ref(false)
const saving = ref(false)
const autoGenerating = ref(false)
const autoGenerateDialogVisible = ref(false)
const autoGenerateMode = ref('PANTRY')
...
const availableRecipes = computed(() => [
  ...recipes.value,
  ...weeklyGeneratedRecipes.value.filter((generated) => !recipes.value.some((recipe) => recipe.id === generated.id))
])
...
function openAutoGenerateDialog() {
  if (loading.value || autoGenerating.value || saving.value) {
    return
  }
  autoGenerateMode.value = recipes.value.length ? 'PANTRY' : 'RANDOM'
...
async function confirmAutoGenerate() {
  ...
  if (autoGenerateMode.value === 'PANTRY' && !recipes.value.length) {
    return
  }
  ...
  try {
    const response = await autoGenerateWeeklyMenu({
      ...
      overwrite,
      mode: autoGenerateMode.value
    })
    applyWeeklyMenu(response.data.data)
    activeTab.value = 'menu'
    ElMessage.success('AI 已生成本周菜单，请检查安排后保存')
  } catch (error) {
...
function applyWeeklyMenu(value) {
  ...
  weeklyGeneratedRecipes.value = normalized.items
    .filter((item) => item.recipeTitle && !recipes.value.some((recipe) => recipe.id === item.recipeId))
    .filter((item, index, items) => items.findIndex((candidate) => candidate.recipeId === item.recipeId) === index)
    .map((item) => ({
      id: item.recipeId,
...
.auto-generate-option:hover,
.auto-generate-option:focus-visible,
.auto-generate-option.selected {
  border-color: var(--app-accent);
  background: var(--app-accent-soft);
  outline: none;
}

.auto-generate-option:disabled {
  color: var(--app-text-muted);
  background: var(--app-surface-soft);
  cursor: not-allowed;
  opacity: 0.62;
...
.auto-generate-option:disabled:hover,
.auto-generate-option:disabled:focus-visible {
  border-color: var(--app-line);
  background: var(--app-surface-soft);
}

.auto-generate-option-mark {
  display: grid;
  place-items: center;
  min-height: 44px;
  border-radius: 6px;
...
.auto-generate-option strong,
.auto-generate-option small {
  display: block;
}
