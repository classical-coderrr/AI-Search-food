...
小厨灵是一款面向家庭日常烹饪的多模态 AI 助手，帮助用户从“冰箱里现有食材”快速走到“可以开做的一餐”。你可以手动输入食材、上传照片或调用摄像头识别；确认识别结果后，系统会结合餐次、口味偏好、饮食限制、健康档案和当前库存，生成多道候选菜谱，并提供食材用量、分步做法、缺料提示和营养估算。

**典型流程：**录入或拍摄食材 → 校对识别结果 → 设定餐次与饮食目标 → 比较菜谱并查看做法 → 收藏菜谱或加入周菜单。

除了单次菜谱推荐，你还可以与厨房 Agent“小厨灵”进行多轮对话，让它查询或生成菜谱、结合库存给出建议，并协助管理收藏和周菜单。涉及新增、修改或消耗个人数据的操作会先展示确认信息；食材库存、健康档案、菜谱和菜单也可以在智能厨房工作台中分别管理。

本项目是基于 Java 17、Spring Boot 3 与 Vue 3 构建的毕业设计，提供 H2 本地体验配置和 Docker Compose 完整部署方案。菜谱生成与图片识别需要配置 DashScope/Qwen 兼容接口的 API Key；具体启动步骤见下方“快速开始”。
